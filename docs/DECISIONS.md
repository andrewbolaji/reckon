...
| 2026-07-16 | Reproducibility | Pin all three generator seeds (Aria=42, Stripe=77, Jobs=99) | Amounts and records are identical across runs. Previously Stripe used global random. MongoDB jobs derived from Aria seed 42 for referential consistency. |
| 2026-07-16 | Security | Sanitize DB error messages | Raw DB errors (column names, table structures) were returned to the user. Now every tool returns a generic message; full detail goes only to the audit log. |
| 2026-07-16 | Security | Separate data from inference in system prompt | On "why" questions the copilot states the "what" from data first, then labels any inferred causes. Never states a cause the warehouse cannot support as fact. |
| 2026-07-16 | Security | Clarify two revenue bases in system prompt | mart_revenue = Stripe payments, mart_jobs.total_completed_value = completed job value. Never divide Stripe revenue by job counts. |
| 2026-07-17 | Observability | All observability behind compose profiles | Prometheus, Pushgateway, Grafana, Loki, Promtail run behind profiles: [observability]. Plain docker compose up is unchanged. make observability is the single command. |
| 2026-07-17 | Observability | OTel instrumentation gated on OTEL_ENABLED | When unset or false, no OTel packages are imported. Zero runtime overhead for the default path. |
| 2026-07-17 | Observability | Pipeline metrics via Pushgateway, non-fatal | Standard Prometheus pattern for batch jobs. Push has a 5s timeout and swallows failures. Observability never breaks the pipeline. |
| 2026-07-17 | Observability | dbt results from run_results.json, not stdout | Structured artifact is reliable across dbt versions. Stdout format is brittle and changes without notice. |
| 2026-07-17 | Observability | Setuptools pinned to <81 in API Dockerfile | OTel FastAPI instrumentor depends on pkg_resources, removed in setuptools 81+. |
| 2026-07-17 | Reproducibility | Fixed REFERENCE_DATE (2026-07-16) for byte-reproducible data | Generators anchor event timestamps to REFERENCE_DATE, not datetime.now(). A fresh docker compose up on any calendar date produces identical mart numbers. Freshness gate still uses real _loaded_at. |
